...
def main(args=None):
    ...
    default_db_path = abspath(join(dirname(__file__), '..', '..', '_reports.db'))
    ...
    parser.add_argument('--db-path', default=default_db_path, help=f"Location to use for sqlite3 db (default {default_db_path})")
    ...
    chain = api.get_chain()
    db = Db(args.db_path, chain, args.denom, args.scale)
    ...
    export_csvs(db, args.csv_path, chain, args.denom, accounts)
